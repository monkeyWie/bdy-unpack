package com.github.monkeywie.bdy;

import com.github.monkeywie.bdy.util.ByteUtil;
import java.io.File;
...
public class BdyUnpack {
  ...
  public static void unzip(String path, String toPath, BdyUnzipCallback callback)
    ...
    try {
      ...
      try (
          FileChannel zipFileChannel = new RandomAccessFile(zipFile, "rw").getChannel()
      ) {
        List<BdyZipEntry> list = getFixedEntryList(zipFileChannel, callback);
        for (BdyZipEntry zipEntry : list) {
          if (callback != null) {
            callback.onEntryStart(zipEntry);
          }
          long fileSize = zipEntry.getCompressedSize();
          File unzipFile = new File(toPath + File.separator + zipEntry.getFileName());
          if (zipEntry.isDir()) {
            if (!unzipFile.exists()) {
              Files.createDirectories(unzipFile.toPath());
            }
          } else {
            if (!unzipFile.getParentFile().exists()) {
              Files.createDirectories(unzipFile.getParentFile().toPath());
            }
            if (unzipFile.exists()) {
              unzipFile.delete();
            }
            ...
            try (
                FileChannel unzipFileChannel = new RandomAccessFile(unzipFile, "rw").getChannel()
            ) {
              zipFileChannel.position(zipEntry.getFileStartPosition());
              ByteBuffer buffer = ByteBuffer.allocateDirect(8192);
              long writeSize = 0;
              while (writeSize < fileSize) {
                zipFileChannel.read(buffer);
                buffer.flip();
                writeSize += buffer.remaining();
                if (writeSize > fileSize) {
                  long overflowSize = writeSize - fileSize;
                  writeSize -= overflowSize;
                  buffer.limit((int) (buffer.limit() - overflowSize));
                }
                unzipFileChannel.write(buffer);
                if (callback != null) {
                  callback.onEntryWrite(zipEntry, fileSize, writeSize);
                }
                buffer.clear();
              }
            }
  ...
  public interface BdyUnzipCallback {
    ...
    void onEntryWrite(BdyZipEntry entry, long totalSize, long writeSize);
  ...
  public static class ConsoleUnzipCallback implements BdyUnzipCallback {
    ...
    public void onFixDone(List<BdyZipEntry> list) {
      ...
      list.forEach(entry -> System.out.println(entry.getFileName()));
      System.out.println("==============================================================");
    }
    ...
    public void onEntryStart(BdyZipEntry entry) {
      if (!entry.isDir()) {
        this.onEntryWrite(entry, 1, 0);
      }
    }

    @Override
    public void onEntryWrite(BdyZipEntry entry, long totalSize, long writeSize) {
      System.out.print("\runpack file:" + entry.getFileName() + "(" + String.format("%.2f", writeSize * 100 / (double) totalSize) + "%)");
    }
